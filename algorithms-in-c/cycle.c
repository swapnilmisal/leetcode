...
int find_set(int key) {
  ...
  if (!head)
    ...
    for (temp_1 = head; temp_1->node->key != key && temp_1;
         temp_1 = temp_1->next)
      ;
    for (tem_1 = temp_1->node; tem_1->parent != tem_1; tem_1 = tem_1->parent)
      ;
    //					printf("represetative of %d is %d",key,tem_1 ->
    //key);
...
void edges(int n) {
  ...
  // int * edge = (int *)malloc(2*n*sizeof(int));
  int i;
...
int cycle() {
  ...
  for (i = 0; i < N; i++)
    if (find_set(edge[i][0]) != find_set(edge[i][1]))
      uni(find_set(edge[i][0]), find_set(edge[i][1]));
    else
      return 1;
